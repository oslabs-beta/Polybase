...
async function configureNeo4jConnection(config) {
  await neo4jPool.runTask(async () => {
    try {
      ...
      logInfo(":heavy_check_mark: Connection to Neo4j established.", { config }, true); // Display in console
...
async function configureMongoConnection(config) {
  ...
  await mongoPool.runTask(async () => {
    try {
      ...
      logInfo(
        ":heavy_check_mark: Connection to MongoDB established.",
...
async function configurePostgresConnection(config) {
  ...
  await postgresPool.runTask(async () => {
    try {
      ...
      logInfo(
        ":heavy_check_mark: Connection to PostgreSQL established.",
        { database: config.database },
        true
      );
      return client;
...
async function configureInfluxConnection(config) {
  await influxPool.runTask(async () => {
    try {
      ...
      logInfo(
        ":heavy_check_mark: Connection to InfluxDB initialized.",
        { url: config.url },
        true
      );
      logInfo(
  ...
  await redisPool.runTask(
    () =>
      new Promise((resolve, reject) => {
        ...
        redis.on("connect", () => {
          logInfo(
            ":heavy_check_mark: Connection to Redis established.",
